...
--      Doc ID:         Note:31412.1    Type:   SCRIPT
--      Last Revision Date:     04-MAR-2008     Status:         PUBLISHED

--> setting sqlplus format
set echo off
set scan on
set lines 150
set pages 100
...
prompt Getting value for table_owner
set termout off
define owner = &1
set termout on

column TABLE_NAME heading "Tables owned by &&owner" format a30


prompt Getting value for table_name
set termout off
define table_name = &2
set termout on


column dcol new_value run_date noprint
select to_char(sysdate,'YYYYMMDDHH24MISS') dcol from dual;

spool show_cbo.&&run_date..&&owner..&&table_name..lst


--> seting columns format
